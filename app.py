    ...
    try:
        with conn.cursor() as cursor:
            ...
            CREATE TABLE IF NOT EXISTS devices (
                ...
                asset_manufacturer VARCHAR(100),
...
def add_device():
    ...
    if request.method == 'POST':
        ...
        asset_manufacturer = request.form['asset_manufacturer']
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute('''
                INSERT INTO devices 
                (asset_number, asset_name, asset_type, asset_manufacturer, purchase_cost, purchase_date, 
                estimated_purchase_date_range, details, asset_condition, storage_type)
...
def predict():
    ...
    for _ in range(1000):
        ...
        noisy_input['Age'] += np.random.normal(0, 0.3)
        X_processed = preprocessor.transform(noisy_input)
        preds.append(regressor.predict(X_processed)[0])
    ...
    return jsonify({
        'max_price': float(ci_upper),
        'mean_price': float(predicted_price),
        'min_price': float(ci_lower),
    })
